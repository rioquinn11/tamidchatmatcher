  ...
  async function handleSearch(e) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/matches/search`, {
        ...
        body: JSON.stringify({ query: searchQuery.trim(), limit: MATCH_POOL_SIZE, email: emailRef.current }),
      });
      const data = await res.json();
      if (data.error) {
          ...
          {SIDEBAR_TABS.map(tab => (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold transition-colors ${
                activeTab === tab.id
                  ? 'bg-[#F0F8FF] text-[#4C9BEA]'
                  : 'text-[#656D79] hover:bg-[#F2F4F8] hover:text-[#2E323A]'
              }`}
            >
              {tab.icon}
              {tab.label}
